...
export function ProjectsTable({ data }: ProjectsTableProps) {
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, _columnId, filterValue) => {
      ...
      const tagline = String(row.original.tagline).toLowerCase()
      const category = String(row.original.category).toLowerCase()
      const techs = (row.original.technologies || []).join(' ').toLowerCase()
      const searchValue = String(filterValue).toLowerCase()

      return (
        id.includes(searchValue) ||
        title.includes(searchValue) ||
        tagline.includes(searchValue) ||
        category.includes(searchValue) ||
        techs.includes(searchValue)
      )
    },
  ...
  return (
    ...
    >
      <DataTableToolbar
        table={table}
        searchPlaceholder='Search by project title, tagline, or tech...'
        filters={[
          {
            columnId: 'category',
            title: 'Category',
